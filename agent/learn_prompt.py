...
def build_learn_prompt(user_request: str) -> str:
    ...
    return (
        ...
        "(action=\"create\"). If `skill_manage` is unavailable on an external "
        "runtime, use `write_file` to create the validated file directly at "
        "`~/.hermes/skills/<category>/<name>/SKILL.md`; do not merely describe "
        "the skill and do not save it in the project workspace. Pick a sensible "
        "category. If the procedure needs a non-trivial script, add it under "
        "the skill's `scripts/` with `write_file` and reference it by relative "
        "path.\n\n"
